...
$chkbox = $_POST['chkbox'];
 ...
 foreach($chkbox as $chkNew1)  
   {  
	...
	$sql = "INSERT INTO orders3 (order_date, client_name,client_contact ,billno,gst,  grand_total, paid, due, payment_type, payment_status, order_status,cgst,sgst,igst,cgsttotal,sgsttotal,igsttotal,aluminium,product1total,product2total,product3total,words,phoneno,address,fright,expense,vehicleno,sub_total,total_amount,discount) VALUES ('$orderDate', '$clientName', '$clientContact ','$billno', '$gst',  '$grandTotalValue', '$paid', '$dueValue',' $paymentType',' $paymentStatus', 1,'$cgst','$sgst','$igst','$cgsttotal','$sgsttotal','$igsttotal','$chkNew','$product1total','$product2total',' $product3total','$word','$phone','$address','$fright','$expense','$vehicle','$fiAmountValue','$totalAmountValue','$discount')";
	...
	for($x = 0; $x < count($_POST['productName']); $x++) {			
		...
		while ($updateProductQuantityResult = $updateProductQuantityData->fetch_row()) {
			$updateQuantity[$x] = $updateProductQuantityResult[0] - $_POST['quantity'][$x];							
				...
				VALUES ('$order_id', '".$_POST['productName'][$x]."', '".$_POST['quantity'][$x]."', '".$_POST['rate'][$x]."', '".$_POST['hsnValue'][$x]."', '".$_POST['yyyValue'][$x]."')";
...
$orderItemSql2 = "INSERT INTO order_item7 (order_id, product_id,hns,quantity,altquantity,rate,total) 
				VALUES ('$order_id', '".$_POST['product1Name'][$y]."', '".$_POST['hsnValue1'][$y]."', '".$_POST['xxx'][$y]."', '".$_POST['ssd'][$y]."','".$_POST['rrd'][$y]."', '".$_POST['vvdValue'][$y]."' )";
