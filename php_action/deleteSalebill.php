...
if($_POST) {	
  ...
  $paymentStatus 				= $_POST['paymentStatus'];
	...
	for($x = 0; $x < count($_POST['productName']); $x++) {			
		...
		$removeOrderSql1 = "DELETE FROM order_item6 WHERE order_id = {$orderId}";
		$connect->query($removeOrderSql1);	
...
$orderItemSql2 = "INSERT INTO order_item7 (order_id, product_id,hns,quantity,altquantity,rate,total) 
		...
		$removeOrderSql2 = "DELETE FROM order_item7 WHERE order_id = {$orderId}";
		$connect->query($removeOrderSql2);
...
$updateProductQuantitySql6 = "SELECT production.altquantity FROM production WHERE production.id = ".$_POST['product2Name'][$z]."";
		...
		$removeOrderSql3 = "DELETE FROM order_item9 WHERE order_id = {$orderId}";
		$connect->query($removeOrderSql3);
 ...
 header("Location: DeletedSale.php");	
	$connect->close();
