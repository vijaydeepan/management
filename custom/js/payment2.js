...
$(document).ready(function() {
	...
	manageProductTable = $('#manageProductTable').DataTable({
		'ajax': 'php_action/fetchsupplierpay.php',
		'order': []
	});
...
function editProduct(brandId = null) {
	...
	if(brandId) {
		...
		$.ajax({
		  ...
		  // 		allowedFileExtensions: ["jpg", "png", "gif", "JPG", "PNG", "GIF"]
				...
				// product id 
				$(".editProductFooter").append('<input type="hidden" name="brandId" id="brandId" value="'+response.product_id+'" />');				
				$(".editProductPhotoFooter").append('<input type="hidden" name="brandId" id="brandId" value="'+response.product_id+'" />');				
					...
					if(productImage) {
						...
						$.ajax({
				          ...
				          // reload the manage student table
									...
									$.ajax({
										url: 'php_action/fetchProductImageUrl.php?i='+brandId,
										type: 'post',
										success:function(response) {
	...
	if(row) {
		var productId = $("#total_quantity2"+row).val();		
		...
		if(productId == "") {
			...
			$.ajax({
				url: 'php_action/fetchSelectedProduct7.php',
				type: 'post',
				...
				success:function(response) {
					...
					$("#total_quantity"+row).val(response.note);

									$("#deepan"+row).val(response.suplier_name);
	...
	function getProduct2(row = null) {
	if(row) {
		var productId = $("#total_quantity21"+row).val();		
		
		if(productId == "") {
			$("#total_quantity1"+row).val("");
			$("#deepan1"+row).val("");
			...
			$.ajax({
				url: 'php_action/fetchSelectedProduct6.php',
				type: 'post',
				data: {productId : productId},
				dataType: 'json',
				success:function(response) {
					...
					$("#total_quantity1"+row).val(response.purchase_quantity);

									$("#deepan1"+row).val(response.product_name);
...
// remove product 
function removeProduct(brandId = null) {
	if(brandId) {
		...
		$("#removeProductBtn").unbind('click').bind('click', function() {
			...
			$.ajax({
				url: 'php_action/removeSuplierspay.php',
				type: 'post',
				data: {brandId: brandId},
				dataType: 'json',
				success:function(response) {
