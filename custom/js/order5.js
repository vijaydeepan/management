...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		manageOrderTable = $("#manageOrderTable").DataTable({
			'ajax': 'php_action/fetchacpOrder.php',
			'order': []
		});		
		...
		if(productId == "") {
			...
			$.ajax({
				...
				success:function(response) {
					...
					$("#dd"+row).val(response.due);

									$("#clientName"+row).val(response.customer_name);

														$("#gstno"+row).val(response.gstno);
...
function addRow(key) {
 if (key.keyCode == "65") {
	...
	$.ajax({
		url: 'php_action/fetchProductData8.php',
		type: 'get',
		dataType: 'json',
		success:function(response) {
	...
	$('.chosen-select').chosen({width: "100%"});
				...
				'</td>'+
					...
					'<input type="number" name="hsn[]" id="hsn'+count+'" autocomplete="off" class="form-control" min="1" />'+
				'</td style="padding-left:20px;">'+
				'<td style="padding-left:20px;"">'+
					'<input type="text" name="alquantity[]" id="alquantity'+count+'" autocomplete="off"  onkeyup="getfoot('+count+')" class="form-control" />'+
	
										'</div>'+

				'</td style="padding-left:20px;">'+
					'<td style="padding-left:20px;"">'+
					'<input type="text" name="foot[]" id="foot'+count+'" autocomplete="off"  onkeyup="getfoot('+count+')" class="form-control" />'+
					'<input type="hidden" name="footValue[]" id="footValue'+count+'" onkeyup="getfoot('+count+')" autocomplete="off" class="form-control" />'+
										'</div>'+
				...
				'</td style="padding-left:20px;">'+
				'<td style="padding-left:20px;"">'+
					'<input type="text" name="yy[]" id="yy'+count+'" autocomplete="off"  class="form-control" />'+
					'<input type="hidden" name="yyValue[]" id="yyValue'+count+'" autocomplete="off" class="form-control" />'+
										'</div>'+
...
function convertNumberToWords(grandTotal) {
    var words = new Array();
    words[0] = '';
    words[1] = 'One';
    words[2] = 'Two';
    ...
    var grandTotal = atemp[0].split(",").join("");
    var n_length = grandTotal.length;
    var words_string = "";
    if (n_length <= 9) {
        var n_array = new Array(0, 0, 0, 0, 0, 0, 0, 0, 0);
        var received_n_array = new Array();
        for (var i = 0; i < n_length; i++) {
            received_n_array[i] = grandTotal.substr(i, 1);
        }
...
function getTotal(row = null) {
	if(row) {
		...
		var total = Number($("#rate"+row).val()) * Number($("#quantity"+row).val()) -Number($("#discount"+row).val()) /100* Number($("#rate"+row).val()) * Number($("#quantity"+row).val());
				var xxx = Number($("#rate"+row).val()) * 10.7639;
 ...
 function gettax() {
	...
	var vat = (Number($("#subTotal").val())/100) * tax;
	$("#vat").val(vat);
	$("#vatValue").val(vat);
	
	var vat2 = (Number($("#subTotal").val())/100) * tax2;
	$("#vat2").val(vat2);
	$("#vatValue2").val(vat2);

		var vat3 = (Number($("#subTotal").val())/100) * tax3;
	$("#vat3").val(vat3);
	$("#vatValue3").val(vat3);
	
	
	var totalAmount = (Number($("#subTotal").val()) + Number($("#vat").val())+ Number($("#vat2").val())+ Number($("#vat3").val()));
...
function removeOrder(orderId = null) {
	if(orderId) {
		$("#removeOrderBtn").unbind('click').bind('click', function() {
			...
			$.ajax({
				url: 'php_action/removeOrder9.php',
				type: 'post',
				data: {orderId : orderId},
				dataType: 'json',
				success:function(response) {
