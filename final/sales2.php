...
function myFunction() {
    alert("The input value has changed. The new value is: " + val);
				...
				$id=$_GET['invoice'];
				include('connect.php');
				$result = $db->prepare("SELECT * FROM sales_bill2 ORDER BY no DESC LIMIT 1 ");
...
<table id="resultTable" data-responsive="table">
	...
	<tbody>
			...
			<?php
				$id=$_GET['invoice'];
				include('connect.php');
				$result = $db->prepare("SELECT * FROM sales_order2 WHERE invoice= :userid");
				$result->bindParam(':userid', $id);
				$result->execute();
				for($i=0; $row = $result->fetch(); $i++){
			?>
	...
	<td colspan="2"><strong style="font-size: 12px; color: #222222;">
				...
				$sdsd=$_GET['invoice'];
				$resultas = $db->prepare("SELECT sum(amount) FROM sales_order2 WHERE invoice= :a");
				...
				$sdsd=$_GET['invoice'];
				$resultas = $db->prepare("SELECT sum(cgsttotal) FROM sales_order2 WHERE invoice= :a");
				$resultas->bindParam(':a', $sdsd);
				$resultas->execute();
				for($i=0; $rowas = $resultas->fetch(); $i++){
				...
				$sdsd=$_GET['invoice'];
				$resultas = $db->prepare("SELECT sum(sgsttotal) FROM sales_order2 WHERE invoice= :a");
				$resultas->bindParam(':a', $sdsd);
				$resultas->execute();
				for($i=0; $rowas = $resultas->fetch(); $i++){
...
<a rel="facebox" href="checkout2.php?pt=<?php echo $_GET['id']?>&total=<?php echo $fgfg ?>&orderDate=<?php echo $_GET['orderDate']?>&cgsttotal=<?php echo $fgfg2 ?>&sgsttotal=<?php echo $fgfg3 ?>&invoice=<?php echo $_GET['invoice']?>&cashier=<?php echo $_SESSION['SESS_FIRST_NAME']?>"><button class="btn btn-success btn-large btn-block"><i class="icon icon-save icon-large"></i> SAVE</button></a>
