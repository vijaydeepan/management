...
$sql = "SELECT order_date, client_name, client_contact, sub_total, vat, total_amount, discount, grand_total,order_id FROM quotation WHERE order_id = $orderId";
...
$quotationorderItemSql = "SELECT quotation_order_item.product_id, quotation_order_item.rate, quotation_order_item.quantity, quotation_order_item.total,
product.product_name FROM quotation_order_item
	INNER JOIN product ON quotation_order_item.product_id = product.product_id 
 WHERE quotation_order_item.order_id = $orderId";
$quotationorderItemResult = $connect->query($quotationorderItemSql);
...
<table border="0" width="100%;" cellpadding="5" style="border:1px solid black;border-top-style:1px solid black;border-bottom-style:1px solid black;">
	...
	<tbody>
		...
		$x = 1;
		while($row = $quotationorderItemResult->fetch_array()) {			
