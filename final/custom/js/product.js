		...
		$.ajax({
		  ...
		  // 		allowedFileExtensions: ["jpg", "png", "gif", "JPG", "PNG", "GIF"]
				...
				$("#editRate").val(response.rate);
								$("#editcgst").val(response.cgst);
								$("#editsgst").val(response.sgst);
				...
				$("#editProductForm").unbind('submit').bind('submit', function() {
                  ...
                  if(cgst == "") {
						$("#editcgst").after('<p class="text-danger">Rate field is required</p>');
						$('#editcgst').closest('.form-group').addClass('has-error');
					}	else {
						// remov error text field
						$("#editcgst").find('.text-danger').remove();
						// success out for form 
						$("#editcgst").closest('.form-group').addClass('has-success');	  	
					}	// /else
						
					
					
						if(sgst == "") {
						$("#editsgst").after('<p class="text-danger">Rate field is required</p>');
						$('#editsgst').closest('.form-group').addClass('has-error');
					}	else {
						// remov error text field
						$("#editsgst").find('.text-danger').remove();
						// success out for form 
						$("#editsgst").closest('.form-group').addClass('has-success');	  	
					}	// /else
					...
					if(productName && quantity && rate && cgst && sgst  && brandName && categoryName && productStatus) {
						// submit loading button
